...
open(FD, "+>$mergedFileDir/mergedErrorbarFile.txt") or die("CRAAAAP! $!");
close(FD);
...
foreach $bonnieFile (@folderFiles) {
 ...
 open (FD, ">>$mergedFileDir/mergedErrorbarFile.txt") or die("Oh well..! $!");
...
open (FD, "<$mergedFileDir/mergedErrorbarFile.txt") or die("AWWW NOOO! $!");
...
plot "$mergedBonniefile" using 0:2:3:4:xticlabels(1) with errorbar lc rgb "#c50000"
EOPLOT
close(GNUPLOT);
close(FD);
